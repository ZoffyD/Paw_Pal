...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
   ...
   if (isset($_GET['search']) && !empty($_GET['search']) && isset($_GET['filter']) && !empty($_GET['filter']) && $_GET['filter'] != 'All') {
        ...
        $sqlsearch = $baseQuery . " AND p.pet_name LIKE '%$search%' AND p.pet_type = '$filter' ORDER BY p.pet_id DESC";

    } else if (isset($_GET['search']) && !empty($_GET['search'])) {
        
        $search = $conn->real_escape_string($_GET['search']);
        $sqlsearch = $baseQuery . " AND p.pet_name LIKE '%$search%' ORDER BY p.pet_id DESC";

    } else if (isset($_GET['filter']) && !empty($_GET['filter']) && $_GET['filter'] != 'All') {
        
        $filter = $conn->real_escape_string($_GET['filter']);
        $sqlsearch = $baseQuery . " AND p.pet_type = '$filter' ORDER BY p.pet_id DESC";
